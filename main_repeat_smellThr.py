...
for smell_thr in range(1, 80, 1):         
    import time
    start_time = time.time()


    import sys
    print_file = "/root/code/ML_groepopdracht3/log/results_smell_thr.txt"
    ...
    import logging
    logging.basicConfig(filename='/root/code/ML_groepopdracht3/log/log_smell_thr.log', filemode='a', format='[%(levelname)s] | %(asctime)s | %(filename)s:%(lineno)d | %(message)s', datefmt='%H:%M:%S %d-%b-%y', level=logging.NOTSET)
