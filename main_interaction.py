...
import sys
print_file = "/root/code/ML_groepopdracht3/log/results_interaction.txt"
...
import logging
logging.basicConfig(filename='/root/code/ML_groepopdracht3/log/log_interaction.log', filemode='a', format='[%(levelname)s] | %(asctime)s | %(filename)s:%(lineno)d | %(message)s', datefmt='%H:%M:%S %d-%b-%y', level=logging.NOTSET)
