# A for loop that is used to experiment with different variables
for look_back_hrs in range(1, 13, 1):         
    import time
    start_time = time.time()


    import sys
    print_file = "/root/code/ML_groepopdracht3/log/results_look_back_hrs.txt"
    ...
    import logging
    logging.basicConfig(filename='/root/code/ML_groepopdracht3/log/log_look_back_hrs.log', filemode='a', format='[%(levelname)s] | %(asctime)s | %(filename)s:%(lineno)d | %(message)s', datefmt='%H:%M:%S %d-%b-%y', level=logging.NOTSET)
    ...
    print(f"look_back_hrs: {look_back_hrs}")
    model_name = "RandomForestClassifier5"

    try:
        model = load(f"/root/code/ML_groepopdracht3/{model_name}.joblib")
    ...
    print("==============================END==============================")
    logging.warning(f"Time to run: {time.time() - start_time}")